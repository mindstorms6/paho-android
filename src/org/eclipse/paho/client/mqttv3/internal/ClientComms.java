...
public class ClientComms {
	 ...
	 * the network calls.
	 */
	public ClientComms(DestinationProvider destinationProvider, MqttClientPersistence persistence, Trace trace, WakeLock wl) throws MqttException {
		this.trace = trace;
	 ...
	 * TODO:Each of these threads should very probably acuire a WL
	 */
	public MqttConnack connect(MqttConnect connect, int connectionTimeoutSecs, long keepAliveSecs, boolean cleanSession) throws MqttException {
		if (connected == false) {
			...
			try {
				networkModule.start();
				receiver = new CommsReceiver(trace, this, clientState, tokenStore, networkModule.getInputStream(),this.wl);
				receiver.start();
			...
			try {
				...
				if (ack == null) {
					...
					//TODO:Android, start as alarm manager keep alive, not a new thread!!!
					connected = true;
